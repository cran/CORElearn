  ...
  double Calibrate::cal(double p) {
        ...
        // expects y filled with predictions(key) and actual scores(value), weight reflect number of such instances (usually 1)
         void Calibrate::binIsoCal(marray<sort3Rec> &y, int noInitialBins) {
            ...
            marray<sort3Rec> yb (interval.filled());
            sort3Rec sr;
            for (int b = 0; b < interval.filled(); ++b) {
                sr.weight = w[b];
                sr.key = interval[b];
                sr.value = calProb[b];
                yb.addEnd(sr);
            }
			yb.setFilled(interval.filled()) ;
            isoRegCal(yb);
        }
         void Calibrate::binningCal(marray<sort3Rec> &y, int noBins) {
			...
			if ( y.filled() < noBins) {
			   ...
			   for (i=0 ; i < y.filled() ; i++) {
				   if (i==y.filled()-1)
                      interval[y.filled()-1] = 1.0;  // sentinel
            ...
            interval[noBins - 1] = 1.0;  // sentinel
            w = binWeight;
         }
           // expects y filled with predictions(key) and actual scores(value), weight reflect number of such instances (usually 1)
         void Calibrate::mergeCal(marray<sort3Rec> &y) {
            sortAndUnify(y); //merges the same by prediction(key)
